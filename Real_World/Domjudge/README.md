...
## Domjudge
Before we continue, a quick overview on how Domjudge works on a high level.
...
in accordance with the developers recommendations, and using the provided Domjudge Docker image 
...
allowing me to work backwards. Instead of asking "I have x vulnerability, what can I do with it?" I could go 
...
The core process within a judgehost is "judgedeamon.main.php", this process poll's the domserver for new submissions, 
...
The second one is the most attractive target as, provided we are able to que new judgings, we have a wide degree of 
...
runguard (written in c) and evict.c. It's possible more bugs exist here that I missed, but given the mentioned 
...
which means the domserver may dispach a job with the submit id
```json
{
    // data
...
Job's are created and queued in the database, the domserver adds jobs into a table and then adds them to a queue. 
...
we're fully worked out, they had been at least identified.
...
Firstly, it turns out the edge-side inclusion requires authentication. I'm not entirely sure why this is, my best guess 
...
would have helped understanding the concept of controllers and security as well as introduce some shortcuts early on 
like the Symfony console.
...
debug setup, however I didn't have an easy way to make modifications, quick prototyping or attaching a debugger. 
...
That concludes this blog post, hope you enjoyed it. If you have any questions or spot any mistakes feel free to hit 
me up on Twitter.
